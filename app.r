...
server <- function(input, output) {
  set.seed(122)
  histdata <- rnorm(500)
  data <- read.csv(file = '/home/rajat/Downloads/mappedData.csv', header = TRUE, fill = TRUE)
  #data = as.data.frame(data)
  
  output$caption <- renderValueBox({
    h <- new_handle()
    handle_setform(h,
                   college = "118",
                   branch = "12",
    ...
    #json = 1
    valueBox(json,"OUTPUT", icon = icon("stats",lib='glyphicon'),color = "purple")  
  })
